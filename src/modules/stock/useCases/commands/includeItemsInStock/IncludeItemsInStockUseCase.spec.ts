...
export const IncludeItemsInStockUseCase = () => {
  let useCase: UseCase;
  let stock: Stock;
  let stockRepository: StockRepository;
  
  it('should be success if include new items in stock', async () => {
    const itemProps = {
  ...
  it('should be fail if stock not exists', async () => {
    const itemProps = {
      name: faker.commerce.productName(),
      quantity: faker.datatype.number(50)
    }
  ...
  it('should be fail if stock not is valid', async () => {
    const itemProps = {
      name: '',
      quantity: faker.datatype.number(50)
    }
